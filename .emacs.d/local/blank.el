...
(defun blank--unfilled-blank-p (pos)
  "Return t if POS points to a non fully filled blank."
  (save-excursion
    (goto-char pos)
    (looking-at-p "[^[:space:]_]*_")))

(defun blank--next-blank (&optional backward ignore-fulfilled-blank)
  "Move point to the next blank.
If there is no blanks after point, move point to the first blank.
if BACKWARD is not nil, operation will be performed in the inverse direction.
This will ignore fully filled blanks if IGNORE-FULFILLED-BLANK is not nil."
  (let ((blanks (if ignore-fulfilled-blank
                    (seq-filter 'blank--unfilled-blank-p blank-blanks)
                  blank-blanks))
        (cmp (if backward '< '>))
        (idx (if backward -1 0)))
    (if blanks
...
(defvar blank-mode-map
  (let ((map (make-sparse-keymap)))
    (define-key map (kbd "TAB") 'blank-next-unfilled-blank)
    (define-key map (kbd "<backtab>") 'blank-previous-unfilled-blank)
